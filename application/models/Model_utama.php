...
class Model_utama extends CI_model {
    ...
    function berita_perkategori($id, $dari, $jumlah) {
        return $this->db->query("SELECT a.*, b.nama_kategori FROM berita a LEFT JOIN kategori b ON a.id_kategori=b.id_kategori where a.id_kategori='$id' AND is_aktif=1 ORDER BY a.id_berita DESC LIMIT $dari, $jumlah");
    }
    ...
    function mainmenu() {
        return $this->db->query("SELECT * FROM mainmenu where aktif='Y' ORDER BY urutan ASC");
    }
    ...
    function semua_berita($start, $limit) {
        return $this->db->query("SELECT * FROM berita a JOIN users b on a.username=b.username WHERE is_aktif=1 ORDER BY id_berita DESC LIMIT $start,$limit");
    }
    ...
    function semua_berita_cari($start, $limit, $kata) {
        ...
        $cari = "SELECT * FROM berita a LEFT JOIN users b ON a.username=b.username LEFT JOIN kategori c ON a.id_kategori=c.id_kategori WHERE a.is_aktif=1 AND is_publik=1 AND ";
    ...
    function berita_detail($id) {
        return $this->db->query("SELECT a.*, b.NAMA AS nama_pegawai, c.* FROM berita a LEFT JOIN remunerasi_pegawai b ON a.username=b.NIP LEFT JOIN kategori c ON a.id_kategori=c.id_kategori where a.id_berita='" . $this->db->escape_str($id) . "' OR a.judul_seo='" . $this->db->escape_str($id) . "'");
    }
    ...
    function detail_kategori($id, $dari, $sampai) {
        return $this->db->query("SELECT a.*, b.NAMA AS nama_pegawai, c.* FROM berita a LEFT JOIN remunerasi_pegawai b ON a.username=b.NIP LEFT JOIN kategori c ON a.id_kategori=c.id_kategori where a.id_kategori='" . $this->db->escape_str($id) . "' AND a.is_aktif=1 ORDER BY a.id_berita DESC LIMIT $dari,$sampai");
    }
    ...
    function info_terkait($limit, $tag) {
        ...
        $jml_katakan = (integer) count($pisah_kata);
        $jml_kata = $jml_katakan - 1;
        $cari = "SELECT * FROM berita LEFT JOIN kategori ON berita.id_kategori=kategori.id_kategori WHERE ";
        for ($i = 0; $i <= $jml_kata; $i++) {
    ...
    function terbaru(){
       ...
       function populer(){
        return $this->db->query("SELECT a.*, b.NAMA AS nama_pegawai, c.* FROM berita a LEFT JOIN remunerasi_pegawai b ON a.username=b.NIP LEFT JOIN kategori c ON a.id_kategori=c.id_kategori WHERE a.is_aktif=1 AND is_publik=1 ORDER BY dibaca DESC limit 4 ");
    }
    ...
    function ambiltag(){
        return $this->db->query("SELECT * FROM tag");
    }
